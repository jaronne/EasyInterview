package com.jaron.easyInterview.service.impl;
...
//        }
    ...
    public void batchAddQuestionsToBank(List<Long> questionIdList, long questionBankId, User loginUser) {
        ...
        ThrowUtils.throwIf(questionBankId <= 0, ErrorCode.PARAMS_ERROR, "非法题库 id");
        ...
        // 合法的题目 id，节约内存，直接查出id，而不是 question 对象
        List<Long> validQuestionIdList = questionService.listObjs(questionLambdaQueryWrapper, obj -> (Long) obj);
        ThrowUtils.throwIf(validQuestionIdList == null, ErrorCode.NOT_FOUND_ERROR, "合法题目列表为空");
        // 剔除已经存在于题库中的题目，参数校验提前，避免重复插入导致数据库操作报错
        ...
        for (int i = 0; i < totalQuestionListSize; i += batchSize) {
            List<Long> subList = validQuestionIdList.subList(i, Math.min(totalQuestionListSize, i + batchSize));
            List<QuestionBankQuestion> collect = subList.stream()
                    .map(questionId -> {
            ...
            CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                questionBankQuestionService.batchAddQuestionsToBankInner(collect);
            }, customExecutor);
            futures.add(future);
        }
        // 等待所有批次完成操作
